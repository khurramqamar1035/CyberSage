...
import { Eye, Zap, ClipboardList, Check, Lock, X } from 'lucide-react';
...
const MyServices = () => {
  ...
  const [popup, setPopup] = useState(null); // service object for request popup
  ...
  const ownedServices = services.filter(s => s.owned);
  const lockedServices = services.filter(s => !s.owned);
  const paidServices = ownedServices.filter(s => s.paymentStatus === 'Paid');
  const unpaidServices = ownedServices.filter(s => s.paymentStatus === 'Unpaid');
  const inProgressCount = ownedServices.filter(s => s.status === 'In Progress').length;
  const completedCount = ownedServices.filter(s => s.status === 'Completed').length;
  ...
  const handleRequestService = async () => {
    try {
      ...
      console.error('[REQUEST SERVICE] Error:', err.message);
      alert(err.message);
    } finally {
      setRequesting(false);
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#13192B] border border-[#1C212E] p-6 rounded-xl flex justify-between items-center">
          <div>
            ...
            <h3 className="text-4xl font-bold text-slate-100">{ownedServices.length}</h3>
          </div>
        ...
        <div className="bg-[#0B0F19] border border-amber-500/20 rounded-2xl p-6 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {unpaidServices.map((service) => (
              <div key={service._id} className="relative p-6 rounded-2xl border border-amber-500/20 bg-amber-500/5">
                ...
                {service.price > 0 && (
                  <p className="text-amber-400 font-bold text-lg mb-4">£{service.price}</p>
                )}
                <Link
