    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-50 bg-slate-900/95 backdrop-blur border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          {/* Logo */}
          <div onClick={() => { navigate("/"); setMenuOpen(false); }} className="cursor-pointer">
            <h1 className="text-lg sm:text-xl font-bold text-white">CyberSage</h1>
          </div>
        ...
        {/* Mobile Menu */}
        <div className={`md:hidden transition-max-height duration-300 ease-in-out overflow-hidden ${menuOpen ? "max-h-64 mt-2" : "max-h-0"}`}>
          <div className="flex flex-col gap-2 py-2 px-2 bg-slate-900 border-t border-slate-800">
            <button onClick={() => { navigate("/about"); setMenuOpen(false); }} className="text-amber-400 w-full text-left py-2 px-2 rounded">About</button>
            <button onClick={() => { navigate("/services"); setMenuOpen(false); }} className="text-slate-300 w-full text-left py-2 px-2 rounded hover:text-white">Services</button>
            <button onClick={() => { navigate("/team"); setMenuOpen(false); }} className="text-slate-300 w-full text-left py-2 px-2 rounded hover:text-white">Team</button>
            <button onClick={() => { navigate("/contact"); setMenuOpen(false); }} className="text-slate-300 w-full text-left py-2 px-2 rounded hover:text-white">Contact</button>
          </div>
        </div>
      </nav>
      ...
      <section className="py-16 sm:py-20 text-center px-4">
        <div className="inline-flex gap-2 items-center px-4 py-2 bg-amber-500/20 border border-amber-500/30 rounded-full mb-6">
          ...
          <span className="text-amber-300 text-xs sm:text-sm">ABOUT CYBERSAGE</span>
        </div>

        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4">Our Story</h1>
        <p className="text-slate-400 text-base sm:text-lg md:text-xl">Born from passion, driven by innovation</p>
      </section>

      {/* TEAM SECTION */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-20">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white text-center mb-10 sm:mb-12">Leadership Team</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-6 sm:gap-8">
          {teamMembers.map(member => (
            <div key={member._id} onClick={() => setSelectedMember(member)} className="cursor-pointer group text-center">
              <div className="rounded-2xl overflow-hidden mb-4">
                <img src={member.image} alt={member.name} loading="lazy" className="w-full aspect-square object-cover group-hover:scale-110 transition duration-300" />
              </div>
              <h3 className="text-white font-semibold text-sm sm:text-base">{member.name}</h3>
              <p className="text-slate-400 text-xs sm:text-sm">{member.position}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-24">
        <div className="text-center mb-10">
          <div className="inline-flex gap-2 px-4 py-2 bg-amber-500/20 border border-amber-500/30 rounded-full mb-4">
            ...
            <span className="text-amber-300 text-xs sm:text-sm">GLOBAL PRESENCE</span>
          </div>
          ...
          {offices.map(office => (
            <Card key={office._id} className="bg-slate-900 border-slate-700">
              <CardHeader>
                ...
                <Badge className="w-fit bg-amber-500/20 text-amber-400">{office.type}</Badge>
      ...
      <Dialog open={!!selectedMember} onOpenChange={() => setSelectedMember(null)}>
        {selectedMember && (
          <DialogContent className="bg-slate-900 border-slate-700 w-[95%] sm:max-w-2xl max-h-[90vh] overflow-y-auto text-white">
            <DialogHeader>
              <DialogTitle className="text-xl sm:text-2xl">{selectedMember.name}</DialogTitle>
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedMember.expertise?.map((skill, i) => (
                  <Badge key={i} className="bg-amber-500/20 text-amber-400">{skill}</Badge>
                ))}
              </div>
            </div>
